...
def load_checkpoint(checkpoint_path, model, optimizer=None, scaler=None):
    ...
    if scaler is not None and 'scaler' in checkpoint_dict.keys():
...
def save_checkpoint(model, optimizer, scaler, learning_rate, iteration, checkpoint_path):
    logger.info("Saving model and optimizer state at iteration {} to {}".format(iteration, 
                                                                                checkpoint_path))
    if hasattr(model, 'module'):
        state_dict = model.module.state_dict()
    else:
